...
public class PlayerStatsDelete {
    ...
    public PlayerStatsDelete(CqlSession session) {
        this.session = session;

        // Prepare delete statements for each table
        String deleteFromAMQuery = "DELETE FROM player_data.player_stats_a_m WHERE stats_type = ?";
        String deleteFromNZQuery = "DELETE FROM player_data.player_stats_n_z WHERE stats_type = ?";
    ...
    public void deleteByStatsType(String statsType) {
        ...
        if (upperStatsType.charAt(0) >= 'A' && upperStatsType.charAt(0) <= 'M') {
            ...
            System.out.println("Deleting from player_stats_a_m table");
        } else if (upperStatsType.charAt(0) >= 'N' && upperStatsType.charAt(0) <= 'Z'){
            deleteStatement = deleteFromNZ;
            System.out.println("Deleting from player_stats_n_z table");
        } else {
